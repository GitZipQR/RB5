 ...
 *
 * HOTFIX:
 * 1) decrypt теперь идёт в ОБРАТНОМ порядке слоёв.
 * 2) decryptedDataClient идемпотентный:
 *    - object/array/number/boolean => возвращает как есть
 *    - string non-base64 => возвращает как есть
 *    - base64 envelope => расшифровывает
 */
...
const getTextEncoderCtor = (): typeof TextEncoder => {
  if (typeof TextEncoder !== 'undefined') return TextEncoder;
  const g = typeof globalThis !== 'undefined' ? (globalThis as any) : null;
  if (g?.TextEncoder) return g.TextEncoder;
  try {
...
const getTextDecoderCtor = (): typeof TextDecoder => {
  if (typeof TextDecoder !== 'undefined') return TextDecoder;
  const g = typeof globalThis !== 'undefined' ? (globalThis as any) : null;
  if (g?.TextDecoder) return g.TextDecoder;
  try {
    const req = eval('require') as any;
    const util = req?.('util');
...
const getEncoder = (): TextEncoder => {
  if (!enc) enc = new (getTextEncoderCtor())();
  return enc;
};

const getDecoder = (): TextDecoder => {
  if (!dec) dec = new (getTextDecoderCtor())();
  return dec;
};

const encode = (value: string): Uint8Array => getEncoder().encode(value);
const decode = (value: Uint8Array): string => getDecoder().decode(value);
...
const isBase64 = (s: string): boolean => {
  ...
  try {
    return toBase64(fromBase64(s)) === s;
  } catch {
    return false;
  }
};
...
const aadFor = (saltLen: number, layerIndex: number): Uint8Array =>
  encode(`${ctxBase}|pbkdf2SaltLen=${saltLen}|layer=${layerIndex}`);
...
function pkcs7Unpad(data: Uint8Array, blockSize = 16): Uint8Array {
  ...
  for (let i = 1; i <= padLen; i++) {
    if (data[data.length - i] !== padLen) throw new Error('Invalid PKCS#7 pad bytes.');
  }
...
async function encryptCTR_HMAC(
...
async function decryptCTR_HMAC(
...
async function encryptCBC_HMAC(
  plaintext: Uint8Array,
  encKey: CryptoKey,
  macKey: CryptoKey,
  iv: Uint8Array,
...
async function decryptCBC_HMAC(
  ciphertext: Uint8Array,
  tag: Uint8Array,
  encKey: CryptoKey,
  macKey: CryptoKey,
...
export const encryptedDataClient = async (data: any, keys: ICryptoKey): Promise<string> => {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    const info = encode(`layer-${i}|alg=${alg}`);
  ...
  return toBase64(encode(JSON.stringify(env)));
};
...
export const decryptedDataClient = async (encryptedData: any, keys: ICryptoKey): Promise<any> => {
  ...
  let jsonStr = '';
  try {
    jsonStr = decode(fromBase64(trimmed));
  } catch {
...
export const serverDecryptedDataFromClient = decryptedDataClient;
export const serverEncryptedDataFromClient = encryptedDataClient;