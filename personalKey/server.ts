...
import { serverDecryptedDataFromClient, serverEncryptedDataFromClient } from './client';
// node-oqs is optional and may be absent in lightweight deployments.
let oqs: any = null;
try {
  const req = eval('typeof require === "function" ? require : null') as any
  const moduleName = ['node', '-oqs'].join('')
  if (req) oqs = req(moduleName)
} catch { /* optional */ }
 ...
 */
export const serverQuantEncryptedData = async (data: any, keys: ICryptoKey): Promise<string> => {
...
export const serverQuantDecryptedData = async (encryptedData: any, keys: ICryptoKey): Promise<any> => {
...
export const quantEncryptedData = async (payload?: any, key?: any) => await serverEncryptedDataFromClient(payload, key)
export const quantDecryptedData = async (payload?: any, key?: any) => await serverDecryptedDataFromClient(payload, key)